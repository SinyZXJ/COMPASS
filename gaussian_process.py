...
class GaussianProcess:
    ...
    def plot(self, y_true, target_id, target_num, target_loc=None, all_pred=None, high_idx=None, agent_loc=None):
        # 将 y_true reshape 成网格形状
        y_true = y_true.reshape(40, 40, -1)
        X0p = self.grid[:, 0].reshape(40, 40)
        X1p = self.grid[:, 1].reshape(40, 40)
        ...
        if target_loc is not None and target_loc[target_id] is not None:
        ...
        if target_id+1 == target_num:
            ...
            if agent_loc is not None:
                for loc in agent_loc:
                    # 为每个 agent 绘制一个圆圈表示其视野(r=0.1)
                    fov = plt.Circle(tuple(loc), 0.08, color='y', fill=False)
